...
//    let initials : String = initialFirstName + initialLastName
    ...
    private lazy var accountKit : AccountKitManager = {
        let ak = AccountKitManager(responseType: .accessToken)
        return ak
    }()
    ...
    private lazy var phoneNumberVerificationVC : AKFViewController & UIViewController = {
       let inputState = UUID().uuidString
        ...
        vc.delegate = self
        vc.uiManager = SkinManager(skinType: .contemporary, primaryColor: UIColor.blue)
        return vc
    }()
